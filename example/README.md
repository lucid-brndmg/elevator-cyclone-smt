...
[How to run these Cyclone specifications?](https://github.com/lucid-brndmg/elevator-cyclone-smt?tab=readme-ov-file#running-cyclone-specification)
...
### Details of Selecting Path Length

Cyclone supports checking or enumerating for a path length such as `check for k` where `k` is the path length. This section further explains the choice of path length `k` under different configurations. 
